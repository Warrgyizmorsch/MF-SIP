...
class FundDetailRepositoryImpl extends FundDetailRepository {
  ...
  @override
  Future<Either<Result<FundDetailEntity>, ApiError>> getFundDetail(
  ...
  @override
  Future<Either<Result<pf.SchemeDetailsEntity>, ApiError>> getPortfolioAnlysis(
    Map<String, dynamic> data,
  ) async {
    try {
      final result = await _fundDetailRemoteDataSource.getPortfolioAnlysis(
        data,
      );
      return result.fold(
        (success) {
        ...
        (error) {
          return Right(
            ApiError(message: 'Portfolio info details failed $error'),
          );
        },
      );
    } catch (e) {
  ...
  ///// ----- Nav History ------ data 
  @override
  Future<Either<Result<NavHistoryResponseEntity>, ApiError>> getNavhistory(
    Map<String, dynamic> data,
  ) async {
    try {
      final result = await _fundDetailRemoteDataSource.getNavHistory(data);
      return result.fold(
        (success) {
          if (success.isSuccess && success.data != null) {
            final result = success.data?.toEntity();
            ...
            return Right(ApiError(message: 'Nav History details Failed'));
          }
        },
        (error) {
          return Right(ApiError(message: 'Nav History details failed $error'));
        },
      );
    } catch (e) {
      return Right(ApiError(message: 'Nav History info  fund list  Failed $e'));
    }
  }
}
